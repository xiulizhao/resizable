(function(root, factory) {
  ...
  var loadDependence = function(fncallback) {
    ...
    if (!window.wwload.resizable) {
      window.wwload.resizable = "wait";
      requirejs.config({
        paths: {
          "resizable": "libs/resizable/resizableColumns" // 示例: libs/qrcodejs/qrcode
        }
      });
      require(["resizable"], function() {
        window.wwload.resizable = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.resizable === "wait") {
      setTimeout(function () {
  ...
  var init = function() {
    ...
    $("body").append('<style> .rc-handle-container{position:relative;} .rc-handle{position:absolute;width:7px;cursor:ew-resize;*cursor:pointer;margin-left:-3px;}</style>');
   ...
   */
  function resizable1($ele){
    $ele.find("table").resizableColumns({});
  }
  ...
  setTimeout(function(){
    $.wwclass.helper.updateProp($ele, "data-x-inited", true);
    $ele.find(".thinkcss").removeClass("hidden");
    resizable1($ele);
  },100);
